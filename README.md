...
| telemetry (default) | Briefing automatically reads direct sibling profile `state.db` files when its active home is `<profiles>/<active-profile>`. It derives only that one `<profiles>` parent and never recurses or scans arbitrary host paths. |
| `telemetry_profile_roots` | Additional read-only profile roots containing `<profile>/state.db`; these are additive to automatic sibling discovery. |
| `telemetry_profile_dbs` | Additional allowlisted `state.db` files. Canonical paths (including symlinks) are read once. Shared rows are deduplicated only by recognized stable global session-ID columns; sources without one remain independently counted and are reported in telemetry diagnostics. Missing, unreadable, or incompatible sources are listed as skipped with a non-sensitive reason. |
